...
namespace Crash.Commands
{
	...
	internal static class SelectionUtils
	{
		...
		internal static bool? GetBoolean(ref bool defaultValue, string prompt, string offValue, string onValue)
		{
			...
			var releaseValue = new OptionToggle(defaultValue, offValue, onValue);
			var releaseIndex = go.AddOptionToggle("Choose", ref releaseValue);
		...
		{
			var getUrl = RhinoGet.GetString(prompt, true, ref value);
		...
		/// <param name="value">The chosen Value, can set a default</param>
		/// <returns>
		///     bool on success, false on cancel or integer < 0</returns>
		internal static bool GetPositiveInteger(string prompt, ref int value)
		{
			var getPort = RhinoGet.GetInteger(prompt, false, ref value);
			if (value <= 0)
			{
				return false;
			}
